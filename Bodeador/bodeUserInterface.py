...
class graphicalInterface:
    def __init__(self):
        ...
        self.HighVin = 0
        self.LowVin = 0
        #self.setFinalVin(self)

        self.HighOffset = 0
        self.LowOffset = 0
        #self.setFinalOffset(self)
    ...
    def placeRadioButtons(self):
        ...
        self.rButton_couplingAC = tk.Radiobutton(self.raiz, text="couple AC", 
                                variable = self.TKvarCoupling, value = AC, command = self.refreshCoupling()).grid(row =3, column = 5)
        self.rButton_couplingDC = tk.Radiobutton(self.raiz, text="couple DC", 
                                variable = self.TKvarCoupling, value = DC, command = self.refreshCoupling()).grid(row =4, column = 5)
        ...
        self.rButton_sweepLog = tk.Radiobutton(self.raiz, text="Logarithmic sweep", 
                                variable = self.TKvarSweepType, value = LOG, command = self.refreshSweepType).grid(row =1, column = 5)
        self.rButton_sweepLinear = tk.Radiobutton(self.raiz, text="Linear sweep", 
                                variable = self.TKvarSweepType, value = LINEAR, command = self.refreshSweepType).grid(row =2, column = 5)
    ...
    def placeEntryText(self):
        ...
        self.nombreExcel_tk = tk.Entry(self.raiz, textvariable = excelName, command = self.setExcelName()).grid(row = 10, column = 6)
    ...
    def placeSliders(self):
        ...
        self.Vin_slider = tk.Scale(self.raiz, from_=0.0, to=20.0, length = 100, orient=HORIZONTAL, 
                        variable = slide_vin, command=self.refreshVin_slider).grid(row = 7, column = 2)
        ...
        self.mVin_slider = tk.Scale(self.raiz, from_=0.0, to=1000.0, length = 100, orient=HORIZONTAL, 
                        variable = slide_mvin, command=self.refreshVin_slider).grid(row = 8, column = 2)
        ...
        self.Voffset_slider = tk.Scale(self.raiz, from_=-10, to=10, length = 100, orient=HORIZONTAL, 
                            variable = slide_voffset, command=self.refreshVoffset_slider).grid(row = 9, column = 2)
        ...
        self.mVin_slider = tk.Scale(self.raiz, from_=-1000, to=1000, length = 100, orient=HORIZONTAL, 
                                variable = slide_mvoffset, command=self.refreshVoffset_slider).grid(row = 10, column = 2)
        ...
        self.nPoints_slider = tk.Scale(self.raiz, from_=0, to=400, length = 100, orient=HORIZONTAL, 
                                        variable = slide_npoints, command=self.refreshNpoints_slider).grid(row = 13, column = 2)

    def refreshStartF_slider(self):
        self.fMin = self.slide_fmin.get()

    def refreshStopF_slider(self):
        self.fMax = self.slide_fmax.get()

    def refreshVin_slider(self):
        self.measData.Vin = self.slide_vin.get() + (self.slide_mvin.get() / 1000)

    def refreshVoffset_slider(self):
    ...
    def placeComboBoxes(self):
        ...
        self.comboTime2Establish = ttk.Combobox(self.raiz, command = self.refreshTime2Establish())
    ...
    def refreshTime2Establish(self):
        self.measData.establish = int(self.comboTime2Establish.get())
